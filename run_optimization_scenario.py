...
def build_master_lp(known_schedules, mandatory_surgeries, optional_surgeries,
    ...
    if not known_schedules:
        ...
        prob.status = pulp.LpStatusOptimal # Mark as solved
        return prob
...
def generate_scenario_data(num_surgeries, num_surgeons, num_days):
    """
    Generates a complete set of scenario parameters.
    """
    print(f"Generating data for {num_surgeries} surgeries, {num_surgeons} surgeons, {num_days} days...")
    ...
    # K_d: ORs per day. At least 2, and scales with surgeons
    K_d = {day: max(2, num_surgeons // 2 + random.randint(0,1)) for day in all_days}
    A_ld = {(surg, day): 480 for surg in all_surgeons for day in all_days} # 8 hours max
    ...
    for i in range(num_surgeries):
        surg_id = 101 + i
        duration = random.choice([60, 90, 120, 180, 240, 300]) # Added 300
        surgeon = random.choice(all_surgeons)
        # Deadlines can be up to 2 days past the planning horizon
        deadline = random.randint(1, num_days + 2) 
        infection = random.choice([0, 0, 0, 1, 2]) # Skew towards non-infectious
...
def print_results_report(all_results):
    """
    ...
    for res in all_results:
        ...
        for sched in res['selected_schedules']:
            ...
            if day not in schedules_by_day:
                 schedules_by_day[day] = [] # Should be initialized, but as a safeguard
            schedules_by_day[day].append(sched)
            
        for day, schedules in schedules_by_day.items():
            if not schedules:
                continue # Don't print days with no scheduled ORs
            print(f"  ORs for {day}:")
...
if __name__ == "__main__":
    ...
    # Define the scenarios to run: (name, num_surgeries, num_surgeons, num_days)
    scenarios_to_run = [
        ("Small", 10, 3, 2),
        ("Small-Medium", 20, 4, 3),
        ("Medium", 40, 5, 5),
    ...
    for name, num_surg, num_surgs, num_days in scenarios_to_run:
        # 1. Generate the scenario data
        scenario_params = generate_scenario_data(num_surg, num_surgs, num_days)
        ...
        # 3. Store results (add back params for detailed printing)
        result['all_days'] = scenario_params['all_days']
        all_results.append(result)
        
    # 4. Print the final report
    print_results_report(all_results)
